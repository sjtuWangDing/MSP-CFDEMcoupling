...
void engineSearchMix::findMpiCell(const base::CITensor1& findMpiCellIDs) const {
  ...
  Foam::vector particleCenterPos = Foam::vector::zero;  // 颗粒中心坐标
  Foam::vector spPos = Foam::vector::zero;              // 标志点坐标
  for (int index = 0; index < cloud_.numberOfParticles(); ++index) {
    // reset findMpiCellIDs
    ...
    if (isInside) {
      ...
      // not found
      if (findMpiCellIDs[index] < 0) {
        int altStartID = -1;
        // 遍历当前颗粒的所有 satellitePoint
        for (size_t i = 0; i < satellitePoints_.size(); ++i) {
          spPos = getSatellitePointPos(index, static_cast<int>(i));
 ...
 */
void engineSearchMix::findExpandexCell(const base::CITensor1& findExpandexCellIDs) const {}
