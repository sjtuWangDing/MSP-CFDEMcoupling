...
cfdemDefineTypeName(mixDense);

cfdemCreateNewFunctionAdder(averagingModel, mixDense);

//! \brief Constructor
mixDense::mixDense(cfdemCloud& cloud) : averagingModel(cloud) {}
 ...
 */
void mixDense::setVectorFieldAverage(volVectorField& valueField, volScalarField& weightField,
    ...
    if (cloud_.checkFineParticle(index) || cloud_.checkMiddleParticle(index)) {
      ...
      for (int subCell = 0; subCell < cloud_.particleOverMeshNumber()[index]; ++subCell) {
        ...
        if (cellID >= 0) {  // cell Found
          ...
          valueField[cellID] += weightP * valueVec;
