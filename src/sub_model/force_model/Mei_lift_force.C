...
cfdemDefineTypeName(MeiLiftForce);

cfdemCreateNewFunctionAdder(forceModel, MeiLiftForce);

/*!
 * \brief Constructor
 * \note The initialization list should be in the same order as the variable declaration
 */
MeiLiftForce::MeiLiftForce(cfdemCloud& cloud)
    : forceModel(cloud),
...
void MeiLiftForce::setForceKernel(const int index, Foam::vector& liftForce) {
  ...
  if (findCellID >= 0) {
    ...
    if (magUr > 0.0 && magVorticity > 0.0) {
      ...
      liftForce = 0.125 * M_PI * rho * Cl * magUr * (Ur ^ vorticity) * diameter * diameter / magVorticity;
      ...
      if (forceSubModel_->verbose() && 0 == index) {
        ...
        Pout << "Cl = " << Cl << endl;
        Pout << "lift force = " << liftForce << endl;
      }
    }
  }
}
...
Foam::vector MeiLiftForce::getBackgroundUfluid(const int index, const int findCellID) const {
  // 背景流体速度
...
Foam::vector MeiLiftForce::getBackgroundVorticity(const int index, const int findCellID) const {
  // 背景流体涡量
...
double MeiLiftForce::getBackgroundVoidFraction(const int index, const int findCellID) const {
  double vf = 1.0;
  if (findCellID >= 0) {
    if (forceSubModel_->interpolation()) {
